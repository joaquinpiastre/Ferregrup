    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {section !== 'home' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: '1px solid #1e1e1e', flexShrink: 0 }}>
          <button className="btn-secondary" style={{ padding: '6px 8px' }} onClick={() => setSection('home')} aria-label="Volver al inicio"><ArrowLeft size={14} /></button>
          <div style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>{TITLES[section]}</div>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {section === 'ruta' && (
          <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
            <RutaDelDia session={session} stops={stops} onChanged={refreshStops} />
          </div>
        )}
        {section === 'pedido' && (
          <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
            <PedidoCalle session={session} orders={orders} onOrderCreated={(o) => setOrders((prev) => [o, ...prev])} onOrderChanged={refreshOrders} />
          </div>
        )}
        {section === 'clientes' && (
          <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
            <ClientesPanel token={session.token} canDelete={false} />
          </div>
        )}
        {section === 'catalogo' && (
          <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
            <CatalogoPanel token={session.token} canEdit={false} />
          </div>
        )}
        {section === 'cobros' && (
          <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
            <CobrosForm session={session} />
          </div>
        )}
      </div>
    </div>
